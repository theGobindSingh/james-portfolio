...
The visual language of neumorphism depends on _minimal contrast_ — the whole aesthetic
collapses if shadows are too dark or too light. But accessibility requirements demand
_minimum contrast_ for interactive components. These are in direct opposition.
...
2. The focus indicator must be _outside_ the element's box-shadow stacking context
...
- Add a keyboard event listener to support `Enter` and `Space` activation
...
_invisible to keyboard users who rely on focus style alone_. A keyboard user tabbing
...
**Risk:** If the only differentiator between states is a shadow change, _and_ that shadow
...
shadows), colorblind users aren't disadvantaged _relative to the shadow itself_ — everyone
