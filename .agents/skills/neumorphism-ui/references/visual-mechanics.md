...
A single drop shadow reads as "floating above the surface" — the element sits on top of
the background. Dual shadows (one light, one dark) on opposite sides read as "embossed
from the same surface" because they imply a light source _hitting_ the material from a
specific angle, which creates both a lit face and a shadowed face on the same object.
...
- Elements are _physically part of_ the same material, not layered on top of it
...
Elevation in neumorphism is not absolute — it is relational. An element is elevated _relative
to its container_. This means:
...
To maintain consistent _apparent_ elevation across elements of different sizes, slightly reduce
